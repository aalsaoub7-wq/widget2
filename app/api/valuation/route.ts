...
// Öka tillåten körtid för Vercel Serverless Functions (max 60s på de flesta planer)
...
const APIFY_TIMEOUT_SEC = process.env.APIFY_TIMEOUT_SEC || "120"; // rekommenderat: 20–30s
...
function buildRunsUrl(extra?: Record<string, string | number | undefined>) {
  ...
  const wait = Math.min(300, Number(APIFY_TIMEOUT_SEC) || 20);
...
export async function POST(req: Request) {
  try {
    ...
    // Svensk skylt (ABC123 eller ABC12A)
